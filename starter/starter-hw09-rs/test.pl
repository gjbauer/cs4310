...
ok(!-e "target/debug/nufs", "no binaries");

system("(cargo clean; cargo run --quiet) > /dev/null");

sub mount {
    system("(cargo run mnt --quiet 2>&1) >> test.log &");
    sleep 1;
}

sub unmount {
    system("(fusermount -u mnt 2>&1) >> test.log");
}
